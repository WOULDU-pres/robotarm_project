    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                #목표 트래킹
                #round((c1[0]+c2[0])/2) <= 이게 x 값
                #round((c1[1]+c2[1])/2) <= 이게 y 값
                ...
                if not (target_servoy>=180 and round((c1[1]+c2[1])/2)<=240 or target_servoy<=0 and round((c1[1]+c2[1])/2)>=240):

                        yservo_pid.SystemOutput = round((c1[1]+c2[1])/2)
...
#subprocess.run(['python', 'detect_final.py', '--weight', 'runs/train/fruit_yolov5s_more_result2/weights/best.pt', '--source', '0'])
#python detect_final.py --weight runs/train/fruit_yolov5s_more_result2/weights/best.pt --source 0